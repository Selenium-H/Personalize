...
const ExtensionPreferencesWindow_PersonalizeExtension = new GObject.Class({
  ...
  createAppMenu: function( ) {
    ...
    menu.append(_("About")+" Personalize",      "prefswindow.about"      );
    ...
    preferencesDialogAction.connect('activate', ()=> {
      ...
      let vbox                 = new Gtk.Box({ orientation: Gtk.Orientation.VERTICAL, margin_top: 30, margin_end: 30, margin_bottom: 30, margin_start: 30 });    
...
const Prefs_PersonalizeExtension = new GObject.Class({
  ...
  _init: function() {
    ...
    this.colorPrefsPrefsWindow = new Gtk.ScrolledWindow({hexpand: true});
    this.colorPrefsPrefsWindow.set_child(this.colorPrefs);
...
const PrefsWindow_PersonalizeExtension = new GObject.Class({
  ...
  _init: function(action) {
    ...
    this.parent({ column_spacing: 40, halign: Gtk.Align.CENTER, margin_top: 0, margin_end: 20, margin_bottom: 20, margin_start: 20, row_spacing: 20 });
    this.addGrids();
    this.action = action;
  ...
  addGrids: function() {
    ...
    this.switchBox0 = new Gtk.Grid({ column_spacing: 30, halign: Gtk.Align.CENTER, margin_top: 10, margin_end: 20, margin_bottom: 20, margin_start: 20, row_spacing: 0  });
    this.switchBox1 = new Gtk.Grid({ column_spacing: 30, halign: Gtk.Align.CENTER, margin_top: 20, margin_end: 20, margin_bottom: 20, margin_start: 20,                  row_spacing: 0  });
  ...
  prefsWA: function( KEY, posX, posY, sbox, space = 1 ) {
    ...
    let prefsSwitchBox = new Gtk.Box({ orientation: Gtk.Orientation.VERTICAL, margin_top: 0, margin_end: 0, margin_bottom: 0, margin_start: 0,hexpand:true});
...
const PrefsWindowForTheme_PersonalizeExtension = new GObject.Class({
  ...
  prefRadio: function(KEY, posX, posY, options, items,space) {
    ...
    for (let i = 0; i < options.length; i++) {
      radio = new Gtk.CheckButton({ label: items[i], group: radio });
...
//    this.fileDataDark = this.fileDataDark.replace("/*Libhandy_Widgets_Dark_Variant_End*/\n", "" );
    ...
    pos = this.fileData.indexOf("/*Border_Curvature*/");
    pos = pos + 20;
    nextPos = this.fileData.indexOf("px",pos);
    accentColorStop = this.fileData.substring(pos, nextPos);
    this.fileData     = this.fileData.replaceAll("/*Border_Curvature*/"+accentColorStop+"px", settings.get_int("window-corner-curvature-radius")+"px");
    ...
    GLib.file_set_contents(this.THEME_PATH+"/gtk-4.0/gtk.css",      this.fileData);
    if(settings.get_boolean("allow-dark")) {
      GLib.file_set_contents(this.THEME_PATH+"/gtk-4.0/gtk-dark.css", this.fileDataDark);
    }
    else {
      if(GLib.file_get_contents(this.THEME_PATH+"/gtk-4.0/gtk-dark.css")[0]) {
        GLib.spawn_command_line_sync("rm "+this.THEME_PATH+"/gtk-4.0/gtk-dark.css");
      }
    }
        
    //Generate GNOME Shell Theme
    this.fileData = String(GLib.file_get_contents( Extension.path+"/theme/gnome_shell_3_40_template.css" )[1]);
  ...
  setPathAndInstallTweakedTheme: function() {
    ...
    this.THEME_PATH = GLib.get_home_dir()+"/.themes/";
    try {
    ...
    try {
      ...
      this.fileData = String(GLib.file_get_contents(this.THEME_PATH+"Adwaita-Personalized/gtk-4.0/gtk.css"));
    }
    catch(error) {
      try {
        GLib.spawn_command_line_sync("mkdir "+this.THEME_PATH);
        GLib.spawn_command_line_sync("mkdir "+this.THEME_PATH+"Adwaita-Personalized");
        GLib.spawn_command_line_sync("mkdir "+this.THEME_PATH+"Adwaita-Personalized/gtk-4.0");
